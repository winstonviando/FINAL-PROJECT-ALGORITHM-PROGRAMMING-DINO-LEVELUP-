...
class Dino(pygame.sprite.Sprite):
    def __init__(self, x_pos, y_pos):
        ...
        self.running_sprites.append(pygame.transform.scale(
            pygame.image.load("Assets/Dino/DinoRun1rev.png"), (80, 100)))
        self.running_sprites.append(pygame.transform.scale(
            pygame.image.load("Assets/Dino/DinoRun2rev.png"), (80, 100)))

        self.ducking_sprites.append(pygame.transform.scale(
            pygame.image.load("Assets/Dino/DinoDuck1rev.png"), (110, 60)))
        self.ducking_sprites.append(pygame.transform.scale(
            pygame.image.load("Assets/Dino/DinoDuck2rev.png"), (110, 60)))
    ...
    def __init__(self, kind, x_pos=1280, y_pos= random.choice([300, 250, 200])):
        ...
        try:
            if kind == "shield":
                ...
                self.image = pygame.transform.scale(pygame.image.load("Assets/Powerups/slow.png"), (64, 64))
            else:
            ...
            if kind == "shield":
                ...
                pygame.draw.rect(self.image, (180, 0, 220, 200), (8, 8, 48, 48))
...
game_over = False
obstacle_timer = 0
obstacle_cooldown = random.choice([2000, 1800, 2200])  # vary spawn time a bit
powerup_timer = 0
powerup_cooldown = 6000  # try spawn powerup every ~6s (subject to random chance)
...
while True:
    ...
    for pu in collected:
        ...
        if pu.kind == "shield":
            shield_active = True
            shield_end_time = now + 3000  # 5 seconds
        elif pu.kind == "jump":
            jump_boost_active = True
            jump_boost_end_time = now + 3000
        elif pu.kind == "double":
            double_points_active = True
            double_points_end_time = now + 3000
    ...
    if game_over:
        ...
        if pygame.time.get_ticks() - obstacle_timer >= obstacle_cooldown:
            # random chance both obstacles and powerups
            ...
            if obstacle_random in range(1, 7):
                ...
                # vary y depending on kind: shield near ground, jump low, slow higher
        ...
        # update score (double score if jump boost? we want double score powerup earlier but not requested; we keep jump boost for jump only)
        ...
        # Ground scroll (ground unaffected by speed_multiplier for cosmetic smoothness)
        ground_x -= game_speed #(no multiplier)  → just use game_speed
